...
1. Инициализирует защищенный браузер с подменой системных отпечатков и флагов автоматизации.
2. Осуществляет переход на главную страницу GitLab и автоматически обрабатывает динамические баннеры куки-файлов.
...
* **Стабильность среды:** Полный отказ от `time.sleep()` в пользу динамических ожиданий элементов через `wait_for(state="visible")` с увеличенными таймаутами до 15 секунд. Это предотвращает "мигание" тестов (*flaky tests*) при нестабильном интернет-соединении.
